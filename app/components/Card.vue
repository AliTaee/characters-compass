...
<template>
  ...
  >
    <header class="border-b border-b-slate-200 dark:border-b-slate-600 p-4">
      <h2 data-testid="card-title" class="text-center text-base md:text-2xl font-bold tracking-wide text-slate-700 dark:text-slate-300 truncate">
        {{ props.title }}
      </h2>
    </header>
    <div class="p-4">
      <NuxtImg
        data-testid="image"
        :src="imageSrc" :alt="props.title" class="w-full object-cover mb-4" :class="[
          imageSize === 'cover' ? 'h-64' : 'h-128',
